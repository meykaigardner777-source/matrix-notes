document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  const PRIMARY_KEY = 'my_notes';
  const LEGACY_KEYS = ['matrix_notes_v3', 'matrix_notes_v2', 'matrix_notes', 'notes', 'matrix_notes_v1', 'matrixNotes'];

  function loadAndMigrateNotes() {
    let loadedNotes = [];
    const existingIds = new Set();

    // 1. Try loading from primary key first
    try {
    ...
    // 2. Scan legacy keys and merge any missing notes
    LEGACY_KEYS.forEach(oldKey => {
      ...
      if (rawLegacy) {
        try {
          ...
          console.warn(`Skipping unparseable legacy key "${oldKey}":`, err);
        }
      }
    });
    ...
    // 4. Save consolidated array back to the active key
    try {
      localStorage.setItem(PRIMARY_KEY, JSON.stringify(loadedNotes));
    } catch (e) {
  ...
  function saveNote() {
    ...
    localStorage.setItem(PRIMARY_KEY, JSON.stringify(notes));
    renderNotesList();
    broadcastSync();
  }
